...
check.time_out = 0
dev = DevInfo("192.168.1.191", "8080", "admin", "admin")
...
def get_status_fun():
    ...
    if 200 == requests_code:
        ...
        # 读取每个端口的iccid，并判断是否存在
        modem_status = {}
        check_ng = 0
        result = True  # 默认所有模块都正常
        for i in range(max_ports):
            modem_imei = None
            modem_imei = result_dict["status"][i]["imei"]
...
def dev_redial_fun():
    httpmsg_cookies = dev.cookie_str
    requests_addr = "http://" + dev.ip + ":" + dev.port + "/goip_send_cmd.html?port=all&op=redial&port=1-4"
    header_dict = {'Content-Type': 'application/json',
                   'Cookie': httpmsg_cookies,
                   'Host': '',
                   'Content-Length': '',
    ...
    if 200 == requests_code:
        logging.info("all modem check ok,switch")
        check.result = 0
